...
var vm = new Vue({
	...
	methods: {
		...
		add: function(){
			vm.showList = false;
			vm.title = "新增";
			vm.user = {usertype:2,departid:0,status:1,roleIdList:[],avatar:null,regionId:null,type:0,role:true,positionNameList: []};
			//获取角色信息
			this.getRoleList();
//			this.getRegionList();
		},
...
//			this.getRegionList();
		...
		saveOrUpdate: function (event) {
			var url = vm.user.userId == null ? "../sys/user/pubsave" : "../sys/user/pubupdate";
			$.ajax({
				type: "POST",
			    url: url,
		...
		getRoleList: function(){
			$.get("../sys/role/pubselect", function(r){
				console.log($)
				vm.roleList = r.list;
			});
		},
		...
		reload: function (event) {
			...
			$("#jqGrid").jqGrid('setGridParam',{ 
                postData:{'username': vm.q.username,'mobile':vm.q.mobile,'regionId':vm.q.regionId,'roleId':vm.q.roleId,'type':vm.q.type},
                page:page
            }).trigger("reloadGrid");
		}
	}
