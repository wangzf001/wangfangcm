...
var vm = new Vue({
	...
	methods: {
		...
		saveOrUpdate: function (event) {
			var url = vm.disinfect.id == null ? "../disinfect/save" : "../disinfect/update";
			if(!vm.disinfect.vendorId){
				vm.disinfect["vendorId"]=$("#vendorId").val();
			}
			if(!vm.disinfect.productId){
				vm.disinfect["productId"]=$("#productId").val();
			}
			if(!vm.disinfect.status){
				vm.disinfect["status"]="0"
			}
			$.ajax({
				type: "POST",
			    url: url,
			    data: JSON.stringify(vm.disinfect),
			    success: function(r){
			    	if(r.errCode === 0){
		...
		getInfo: function(id){
			$.get("../disinfect/info/"+id, function(r){
				$("#productIds").hide();
				$("#addressId").hide();
				$("#vendoeId").hide();
				$("#gridId").hide();
				$("#orderId").hide();
					vm.disinfect = r.disinfect;
    			});
		},
